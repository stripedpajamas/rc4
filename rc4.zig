const std = @import("std");

const File = std.fs.File;

pub const RC4 = struct {
    state: [256]u8 = [_]u8{
        ,
    },
    ks: ?Iterator = null,

    pub fn init(key: []const u8) RC4 {
        var rc4 = RC4{};
        var i: usize = 0;
        var j: usize = 0;
        var t: u8 = 0;
        while (i < 256) : (i += 1) {
            j = (j + rc4.state[i] + key[i % key.len]) % 256;
            rc4.swap(i, j);
        }
        return rc4;
    }

    pub fn encrypt(rc4: *RC4, dest: []u8, src: []const u8) void {
        std.debug.assert(dest.len >= src.len);
        var ks = rc4.keystream();
        for (src) |val, idx| {
            dest[idx] = ks.next() ^ val;
        }
    }

    pub fn stream(comptime WriterType: type, comptime ReaderType: type) type {
        return struct {
            rc4: RC4,

            const Self = @This();

            pub fn init(key: []const u8) Self {
                return Self{
                    .rc4 = RC4.init(key),
                };
            }
            pub fn encrypt(self: *Self, dest: WriterType, src: ReaderType) !void {
                var ks = self.rc4.keystream();
                while (src.readByte()) |src_byte| {
                    try dest.writeByte(ks.next() ^ src_byte);
                } else |err| {
                    switch (err) {
                        error.EndOfStream => {
                            return;
                        },
                        else => {
                            return err;
                        },
                    }
                }
            }

            pub const decryptStream = encryptStream;
        };
    }

    pub const decrypt = encrypt;

    fn swap(rc4: *RC4, this: usize, that: usize) void {
        var temp = rc4.state[this];
        rc4.state[this] = rc4.state[that];
        rc4.state[that] = temp;
    }

    pub fn keystream(rc4: *RC4) Iterator {
        if (rc4.ks) |it| {
            return it;
        }
        rc4.ks = Iterator{ .rc4 = rc4 };
        return rc4.ks.?;
    }

    pub const Iterator = struct {
        rc4: *RC4,
        i: usize = 0,
        j: usize = 0,

        pub fn next(it: *Iterator) u8 {
            it.i = (it.i + 1) % 256;
            it.j = (it.j + it.rc4.state[it.i]) % 256;
            it.rc4.swap(it.i, it.j);
            var ks_idx = @as(usize, @as(usize, it.rc4.state[it.i]) + @as(usize, it.rc4.state[it.j]));
            return it.rc4.state[ks_idx % 256];
        }
    };
};

test "rc4" {
    const TestCase = struct {
        key: []const u8, plaintext: []const u8, ciphertext: []const u8
    };
    var test_cases = [_]TestCase{
        .{
            .key = "Key",
            .plaintext = "Plaintext",
            .ciphertext = &[_]u8{ 0xBB, 0xF3, 0x16, 0xE8, 0xD9, 0x40, 0xAF, 0x0A, 0xD3 },
        },
        .{
            .key = "Wiki",
            .plaintext = "pedia",
            .ciphertext = &[_]u8{ 0x10, 0x21, 0xBF, 0x04, 0x20 },
        },
        .{
            .key = "Secret",
            .plaintext = "Attack at dawn",
            .ciphertext = &[_]u8{ 0x45, 0xA0, 0x1F, 0x64, 0x5F, 0xC3, 0x5B, 0x38, 0x35, 0x52, 0x54, 0x4B, 0x9B, 0xF5 },
        },
    };

    var out: [100]u8 = undefined;
    for (test_cases) |tc| {
        var rc4 = RC4.init(tc.key[0..]);
        rc4.encrypt(&out, tc.plaintext[0..]);

        std.testing.expectEqualSlices(u8, out[0..tc.plaintext.len], tc.ciphertext[0..]);
    }
}

test "rc4 streaming" {
    var out = std.ArrayList(u8).init(std.testing.allocator);
    defer out.deinit();
    var writer = out.writer();
    var reader = std.io.fixedBufferStream("Plaintext").reader();

    var rc4 = RC4.stream(@TypeOf(writer), @TypeOf(reader)).init("Key"[0..]);
    try rc4.encrypt(writer, reader);

    std.testing.expectEqualSlices(u8, out.items, &[_]u8{ 0xBB, 0xF3, 0x16, 0xE8, 0xD9, 0x40, 0xAF, 0x0A, 0xD3 });
}
